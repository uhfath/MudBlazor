...
namespace MudBlazor
{
    ...
    public partial class MudTable<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicParameterlessConstructor)] TTableRow> : MudTableBase
    {
        /// <summary>
        ...
        internal override async Task FireRowClickEventAsync(MouseEventArgs args, MudTr row, object o)
        {
            ...
            await OnRowClick.InvokeAsync(new TableRowClickEventArgs<TTableRow>()
            {
                MouseEventArgs = args,
                Row = row,
                Item = item,
        ...
        /// </summary>
        [Parameter] public EventCallback<TableRowHoverEventArgs<TTableRow>> OnRowMouseEnter { get; set; }
        ...
        internal override async Task FireRowMouseEnterEventAsync(MouseEventArgs args, MudTr row, object o)
        {
            ...
            await OnRowMouseEnter.InvokeAsync(new TableRowHoverEventArgs<TTableRow>()
            {
                MouseEventArgs = args,
                Row = row,
                Item = item,
        ...
        /// </summary>
        [Parameter] public EventCallback<TableRowHoverEventArgs<TTableRow>> OnRowMouseLeave { get; set; }
        ...
        internal override async Task FireRowMouseLeaveEventAsync(MouseEventArgs args, MudTr row, object o)
        {
            ...
            await OnRowMouseLeave.InvokeAsync(new TableRowHoverEventArgs<TTableRow>()
            {
                MouseEventArgs = args,
                Row = row,
                Item = item,
        ...
        public TTableRow SelectedItem
        {
            ...
            set
            {
                if (_comparer != null && _comparer.Equals(SelectedItem, value))
                    return;
                else if (EqualityComparer<TTableRow>.Default.Equals(SelectedItem, value))
                    return;
                _selectedItem = value;
            ...
            get
            {
                if (!MultiSelection)
                    if (_selectedItem is null)
                        return new HashSet<TTableRow>(Array.Empty<TTableRow>(), _comparer);
                    else
                        return new HashSet<TTableRow>(new TTableRow[] { _selectedItem }, _comparer);
                else
            ...
            set
            {
                ...
                if (value == null)
                {
                    if (Context.Selection.Count == 0)
                        return;
                    Context.Selection = new HashSet<TTableRow>(_comparer);
                }
                else
        ...
        /// </summary>
        protected bool IsCheckedRow(TTableRow item) 
        ...
        private void OnRowCheckboxChanged(bool checkedState, TTableRow item)
        {
            if (checkedState)
        ...
        internal void OnGroupHeaderCheckboxClicked(bool checkedState, IEnumerable<TTableRow> items)
        {
            if (checkedState)
            {
